...
def generate_master_content(keyword, target_blog_url, scraped_data, title_guide, context_posts, related_keyword, skeleton_title):
    ...
    # -------------------------------------------------------------------
    # 👨‍🎨 [1단계 API] 메인 작가 AI (기현님 오리지널 프롬프트 100% 반영 + 모방 방지)
    # -------------------------------------------------------------------
    prompt_writer = f"""
[타겟 키워드]: {keyword}
...
**🌟 추가 지시 (모방 및 진화):** 위 10개 블로그 데이터를 모방하되, 원본에 있는 특정 날짜, 개인적 에피소드, 실명 등은 100% 가공하여 '나만의 새로운 경험담'처럼 자연스럽게 변형하라. 이전 글을 복사한 느낌이 나지 않도록 더 발전된 정보로 채워라.
...
"""
    ...
    # -------------------------------------------------------------------
    # 🕵️‍♂️ [2단계 API] 편집장 AI (기현님표 4단콤보 제목 포장 + 지능형 지도 + JSON)
    # -------------------------------------------------------------------
    prompt_editor = f"""
...
**[🎯 제목(title) 생성 특명 - 기현님표 4단 콤보 공식 엄수!]**: 
아래 4단계 공식을 무조건 조합하여 클릭률 폭발하는 제목을 만들어라. 단, 뼈대 제목 [{skeleton_title}]의 인기 요소를 살짝 녹여라.
* 1단계 [시선집중 서두]: "2026년 최신판!", "이번 달 여행 준비,", "찐후기 주의,", "현지인만 아는", "결론부터 말하면" 중 택 1
* 2단계 [뾰족한 핵심 훅]: 본문의 구체적인 Niche 주제 자극적 표현
...
**[🎯 초정밀 지능형 지도(map_location) 하네스 규칙]**:
본문을 읽고 가장 가고 싶어 할 실물 장소를 단답형으로 지정하라.
1. 해외 현지: "간사이 국제공항", "다낭 한시장" 등 랜드마크.
2. 국내 출국: "인천공항 마티나 라운지", "신라면세점 서울점", "서울역 도심공항터미널" 등.
3. 온라인/무형: "명동 환전소", "인천공항 제1여객터미널", "인천공항 로밍센터", "외교부 여권민원실" 중 적합한 곳.

[출력 형식 가이드]: 순수 JSON 문자열만 반환하라.
JSON Keys: 
- title: 4단 콤보가 적용된 완벽한 어그로 제목
- meta_desc: 150자 요약
