...
# ==================== [1] 기본 설정 ====================
# 🔥 구글 SDK 지원 종료 에러 방지를 위한 REST API 다이렉트 통신 사용
...
def get_recent_posts(service, blog_id):
    """🔥 내부링크 자동 삽입을 위해 내 블로그의 최근 글 10개를 가져옴"""
    try:
...
def get_content_chain(keyword):
    ...
    for key, flows in CONTENT_FLOW.items():
        if key in keyword:
            return flows[0]  # 첫번째를 항상 메인 브릿지로
    return "여행 준비물"

def get_related_posts_by_keyword(posts, keyword):
    """🔥 내부링크 관련성 필터링 (키워드가 포함된 찐 연관글만 추출)"""
...
def get_naver_target_data():
    ...
    # 🔥 20% 확률로 이전 키워드 재활용 (망한 글 살리기 & SEO 누적)
    ...
    # 순환: 처리한 키워드를 맨 뒤로 보냄 (재활용된 키워드가 아닐 때만)
    ...
    try:
        ...
        # 15개 중 10개 믹스 로직 (1개 메인 뼈대 + 9개 리얼 후기)
...
[미션]: 독자가 글을 읽고 즉시 '결정'을 하도록 유도하는, 4,000~6,000자 분량의 밀도 높은 99점짜리 전환형 포스팅을 작성하라. 

[🔥 품질 필터 - 절대 엄수]:
- 블로그 글 서론에 검색 방법, 조사 과정, 날짜 등 군더더기는 절대 쓰지 말고, 바로 독자에게 필요한 혜택과 정보로 시작해야 한다.
- 모든 문장에는 "왜?"에 대한 해답이 있어야 한다. 단순 설명 금지! 반드시 "이유 + 결과"를 명확히 포함하라.
- 같은 표현의 반복을 피하고 억지로 글자 수만 늘리지 마라. 밀도가 핵심이다.

[🔥 99점 도달을 위한 필수 지시사항]:
1. **후킹 문장 강제 (Hook)**: 서론의 첫 문장은 반드시 "손해", "시간", "돈" 중 하나의 단어를 직접 포함하여 강렬하게 시작하라.
2. **전략적 콘텐츠 연결 (SEO 브릿지)**: 이 글은 반드시 다음 주제와 자연스럽게 연결되어야 한다: [{related_keyword}]. 해당 키워드를 문맥 속에 녹이고, 하단이 아닌 본문 중간에 자연스럽게 <a> 태그로 링크를 걸어라.
3. **문맥형 내부링크 핀셋 삽입**: 위 [내 블로그의 다른 글 리스트]와 연관된 내용이 나오면 문장 중간에 자연스럽게 <a> 태그를 사용하여 관련성 높은 링크를 걸어라. 
4. **결정 버튼 (Decision Maker)**: 글 마지막에 '<h3>결론: 그래서 뭐 쓰라고? (상황별 추천)</h3>' 섹션을 무조건 넣어 독자의 선택을 확정 지어줘라.
5. **숫자와 손해 회피 심리 자극**: 비용(원)과 시간(분)을 명확한 숫자로 비교하여 손해 회피 심리를 자극하라.
6. **외부 링크 최적화 (1~2개 제한)**: SEO 점수 관리를 위해 외부 링크는 딱 1~2개만(구글맵 또는 공식 정보) 핵심적인 곳에 배치하라. (target="_blank" 필수, 구글맵 띄어쓰기는 '+'로 치환)
...
"""
    ...
    for attempt in range(max_retries):
        try:
            print(f"✍️ [4/6] {current_persona} 모드로 4,000~6,000자급 전환형 원고 생성 중... (시도 {attempt + 1})")
...
def run_automation():
    ...
    # 🔥 내부링크 관련성 필터링 적용 (관련 글이 없으면 전체 전달)
    ...
    # HTML 조립 (서론 -> 목차 -> 관련글 -> SVG -> 상단광고 -> 본문 -> 중간/하단광고)
    ...
    # 🔥 광고 위치 추가 최적화 (첫 번째 h2 바로 위 + 중간 h2 위 + 하단)
    content = re.sub(r'(<h2)', ads_code + r'\1', content, count=1) # 첫 번째 h2 상단
    
    h2_parts = content.split("<h2") 
    if len(h2_parts) >= 4: # 이미 상단에 하나 추가되었으므로 4 이상 체크
        mid_index = len(h2_parts) // 2
        h2_parts[mid_index] = ads_code + "<h2" + h2_parts[mid_index]
        content = "<h2".join(h2_parts)
        
    content = content + ads_code # 하단 광고
    ...
    <style>
        ...
        .entry-content {{ font-size: 18px; line-height: 2.1; color: #333; font-family: 'Malgun Gothic', sans-serif; }} 
