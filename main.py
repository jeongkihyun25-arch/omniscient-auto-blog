...
# ==================== [4] 유동적 SVG 요약 카드 (🔥 썸네일 가독성 극대화) ====================
def create_summary_card_tag(summary_list, alt_text):
    # 🚨 AI가 요약(리스트)을 빼먹거나 에러를 냈을 때 파이썬이 뻗지 않도록 튼튼한 방어막 추가
    if not isinstance(summary_list, list):
        summary_list = ["📌 핵심 요약", "💰 비용 절약", "✅ 꿀팁 방출"]

    # 🔥 기현님 기획대로 썸네일 최적화를 위해 짧게 컷! (이모지 포함 10글자 내외)
    safe_list = [str(s).strip()[:10] for s in summary_list if s][:3]
    while len(safe_list) < 3: safe_list.append("") 
    
    # 🔥 font-size를 24 -> 32로 대폭 키우고 줄 간격(y좌표)을 예쁘게 정렬
    svg_code = f"""
    <svg width="100%" height="200" viewBox="0 0 600 200" xmlns="http://www.w3.org/2000/svg">
      <rect width="600" height="200" fill="#FFF9C4" rx="15"/>
      <text x="50%" y="60" font-family="'Apple SD Gothic Neo', 'Malgun Gothic', sans-serif" font-weight="bold" font-size="32" text-anchor="middle" fill="#2c3e50">{safe_list[0]}</text>
      <text x="50%" y="110" font-family="'Apple SD Gothic Neo', 'Malgun Gothic', sans-serif" font-weight="bold" font-size="32" text-anchor="middle" fill="#2c3e50">{safe_list[1]}</text>
      <text x="50%" y="160" font-family="'Apple SD Gothic Neo', 'Malgun Gothic', sans-serif" font-weight="bold" font-size="32" text-anchor="middle" fill="#2c3e50">{safe_list[2]}</text>
    </svg>
    """
    b64_svg = base64.b64encode(svg_code.encode('utf-8')).decode('utf-8')
    
    # 🚨 마크다운 절대 금지! 100% HTML <img> 태그로만 반환하여 그림 깨짐 원천 차단
...
# ==================== [5] 원고 생성 (🔥 기현님 오리지널 글맛 유지 + 2단계 에러방어/지도/모방진화 추가) ====================
def generate_master_content(keyword, target_blog_url, scraped_data, title_guide, context_posts, related_keyword, skeleton_title):
    ...
    # 🔥 수정: 메뉴판 번호표 부여 (AI가 문맥에 [내부링크_0] 형태로 넣기 위함)
    menu_posts = context_posts[:10] 
    recent_titles_str = "\n".join([f"- [내부링크_{i}] 제목: {p['title']} (URL: {p['url']})" for i, p in enumerate(menu_posts)])

    # -------------------------------------------------------------------
    # 👨‍🎨 [1단계 API] 메인 작가 AI (기현님 오리지널 프롬프트 100% 반영 + 강력한 모방 진화)
    # -------------------------------------------------------------------
    prompt_writer = f"""
[타겟 키워드]: {keyword}
...
**🌟 강력한 모방 및 진화 지시:** 위 10개 블로그 데이터를 베이스로 삼되, 원본에 노출된 '특정 날짜, 타인의 실명, 개인적인 에피소드'는 100% 가공하여 네 페르소나에 맞는 **'새로운 나의 경험담'**으로 완전히 탈바꿈시켜라. 경쟁 글의 장점은 흡수하되, 남의 글을 복사한 티가 절대 나면 안 된다. 더 디테일하고 발전된 형태의 정보를 제공하라.

[🔥 핵심 강제 지시사항 - 순수 HTML만 반환 (JSON 포장 절대 금지)]:
...
2. **문맥형 최신 내부링크 (무조건 3개 자연 삽입 강제)**: 
   - 'Related:' 같은 단독 문단 절대 금지. 문장 안에 [내 블로그 다른 글 리스트] 중 3개를 골라 자연스럽게 버튼 스타일로 녹여라.
   - 🚨 직접 <a>태그를 만들지 말고 반드시 `자세한 내용은 [내부링크_0]를 참고하세요.` 와 같이 기호로만 적어라.
...
4. **🔥무조건 접속되는 오류없이 안전한 구글 외부 링크 (무조건 문맥에 맞는 링크 3개 강제)**: 
   - 가짜 오류 URL 방지를 위해, 모든 외부 링크는 **반드시 구글 검색 결과 URL** 또는 **구글 맵 검색 URL**로만 정확히 3개 작성하라.
...
6. **표(Table)와 리스트(List) 절대 엄수 (비교 시 무조건 표 사용)**: 
   - 제품 비교나 장단점 설명 시 절대 줄글로 쓰지 말고, 반드시 `<div class='table-wrapper'><table>...</table></div>` 형태의 완벽한 HTML 표로 작성하라. 
   - 텍스트 나열은 `<p>` 대신 `<ul><li>...</li></ul>`를 적극 활용해라.
7. **JSON 에러 파괴 방지**: 본문 안의 모든 HTML 속성은 반드시 **작은따옴표(')**만 써라! (예: `class='ext-link'`)
8. 🚨 **최고 중요 경고**: 글 맨 끝에 네 대답이나 부연 설명("네 알겠습니다", "작성 완료" 등)을 단 한 글자도 적지 마라! 오직 HTML 소스코드만 출력하라!

[🌟 품질 필터 및 휴먼 톤(Human Tone) 작성 지침]:
- 기계식 서론 금지: 검색 과정 등 군더더기 금지. 곧바로 핵심 혜택 시작.
- 리얼리티 경험담: 친한 회원에게 썰 푸는 말투(~했어요, 솔직히 말해서 등) 사용.
- 완벽한 정보 금지: 일부는 경험처럼 풀고, 반말 느낌이나 감탄문 1~2개 허용.
- 이유+결과 공식: 모든 문장은 "이유+결과"를 적어라.
- 문장 길이 섞기, 말투 살짝 흔들기, 반복 패턴 깨기, 이름이나 너무 개인정보는 밝히지 말것, 모방 사이트 이름 금지.
"""

    print(f"✍️ [4/6] 1단계: 메인 작가 AI가 기현님 스타일의 5,000자 본문을 창작 중입니다...")
    draft_html = None
    ...
    if not draft_html: return None

    # -------------------------------------------------------------------
    # 🕵️‍♂️ [2단계 API] 편집장 AI (1등 SEO 퓨전 + 초정밀 능동형 지도 + JSON)
    # -------------------------------------------------------------------
    prompt_editor = f"""
당신은 완벽한 SEO 편집장입니다. 아래 작성된 [초안 블로그 본문]을 읽고, 메타데이터 JSON을 생성하라.

[초안 블로그 본문]:
{draft_html[:3000]} ... (중략)

**[🔥 최종 제목(title) 생성 특명 - 기현님표 4단 콤보 + 네이버 1등 SEO 퓨전!]**: 
아래 4단계 공식을 무조건 조합하여 클릭률 폭발하는 제목을 만들어라. 
🚨 단, 제공된 네이버 상위노출 뼈대 제목 **[{skeleton_title}]** 에서 사람들의 클릭을 유도한 '매력적인 요소(숫자, 자극적 키워드 등)'를 추출하여 너의 4단 콤보 제목에 자연스럽게 퓨전(융합)시켜라!
* 1단계 [시선집중 서두]: "2026년 최신판!", "이번 달 여행 준비,", "찐후기 주의,", "현지인만 아는", "결론부터 말하면" 중 택 1
* 2단계 [뾰족한 핵심 훅]: 본문의 구체적인 Niche 주제 + 뼈대 제목([{skeleton_title}])의 매력 요소 융합
* 3단계 [신뢰/감정 괄호]: "(직접 해본 결과)", "(Feat. 내돈내산)", "(실제 후기 포함)", "(부작용 주의)", "(정리 끝판왕)" 중 택 1
* 4단계 [클릭 유도 마무리]: "확인해 보세요 🚀", "지금 바로 보기", "전격 비교!", "이 글 하나로 끝내세요" 중 택 1
🚨 [생존 규칙]: 위 제목 안에 타겟 키워드 '{keyword}'는 반드시 원형 그대로 1회 이상 들어가야 한다!

**[🎯 초정밀 능동형 지도(map_location) 하네스 규칙]**:
본문을 읽고 독자가 가장 길을 찾고 싶어 할 실물 장소를 '능동적으로 추론'하여 단답형으로 지정하라.
1. 해외 현지 (맛집/관광): "간사이 국제공항", "다낭 한시장", "신주쿠역" 등 해당 지역의 핵심 랜드마크.
2. 국내 출국 (편의시설): "인천공항 마티나 라운지", "신라면세점 서울점", "서울역 도심공항터미널" 등 конкрет한 시설명.
3. 온라인/무형 (환전/보험/통신/비자): "명동 환전소", "인천공항 제1여객터미널", "인천공항 로밍센터", "외교부 여권민원실" 등 서비스와 직결되는 대표 오프라인 장소.
* 스쳐 지나가는 화장실, 수영장, 단순 도로명 등은 절대 금지.

[출력 형식 가이드]: 순수 JSON 문자열만 반환하라.
JSON Keys: 
- title: SEO 퓨전이 적용된 4단 콤보 완벽 어그로 제목
- meta_desc: 150자 요약
- meta_keys: '{keyword}'가 포함된 쉼표 구분 키워드
- slug: 영문 짧은 주소
- summary: ["✈️ 짧은단어", "💰 짧은단어", "✅ 짧은단어"] (6글자 이하 3개)
- map_location: 능동적 추론 장소명 1개
- category: ["여행 교통 팁", "여행 쇼핑 팁", "여행 관광 팁", "여행 준비 팁", "여행 맛집 팁", "생활 정보 꿀팁"] 중 1개
"""

    print(f"✨ [5/6] 2단계: 편집장 AI가 기현님표 제목과 메타데이터(JSON)를 포장 중입니다...")
    final_data = None
    for attempt, model_name in enumerate(models_to_try, 1):
        api_url = f"https://generativelanguage.googleapis.com/v1beta/models/{model_name}:generateContent?key={GEMINI_API_KEY}"
        try:
    ...
    # -------------------------------------------------------------------
    # 🐍 [3단계] 파이썬 조립 (🔥 수정: 내부링크를 진짜 파란색 버튼 태그로 치환)
    # -------------------------------------------------------------------
